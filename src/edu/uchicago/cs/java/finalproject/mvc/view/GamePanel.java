...
import edu.uchicago.cs.java.finalproject.mvc.model.Meter;
...
public class GamePanel extends Panel {
	...
	//method to get the mouseEvent
	public MouseEvent getMouseEvent() {
		return mMouseEvent;
	}

	//method to set the mouseEvent
	...
	public void update(Graphics g) {
		...
		// Fill in background with black. CLEAR THE SCREEN
		...
		//playing and not paused!
            ...
            //call methods to draw left ships and left shields
			drawNumberShipsLeft(grpOff);
			drawNumberShieldsLeft(grpOff);
			Meter.getInstance().draw(grpOff);
			if (Cc.getInstance().isGameOver()) {
				Cc.getInstance().setPlaying(false);
				//bPlaying = false;
			}
		}
		//call method to draw a moved mouse
		drawTarget(getMouseEvent());
		//draw the double-Buffered Image to the graphics context of the panel
		g.drawImage(imgOff, 0, 0, this);

	} 

	//method to draw the moved mouse
	public void drawTarget(MouseEvent e){
		...
		if (e!= null) {
			...
			//draw circle around where the mouse moved to
			grpOff.drawOval(nX - rad/2, nY - rad/2, rad, rad);
		}
	...
	// Draw the number of shields left on the bottom-left of the screen.
	private void drawNumberShieldsLeft(Graphics g) {
		int shieldNum = Cc.getInstance().getFalcon().getShield();
		//set the color to CYAN
		g.setColor(Color.CYAN);
		//for each Shield left (not including the one that is playing)
		for (int nD = 0; nD < shieldNum; nD++) {
			//create x and y values for the objects to the bottom left
			g.drawOval(20 + 30 * nD, Game.DIM.height - 60, 20, 20);
		}
	}
	...
	private void displayTextOnScreen() {
		...
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4 - 80);

		strDisplay = "use the arrow keys to turn (left and right), thrust(up) and stop(down)";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 120);
		...
		strDisplay = "'M' to TurnOn/Mute Music";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 40);
		...
		strDisplay = "'A' for Timed And Hit-Weakened Shield";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 200);

		strDisplay = "'F' for Hit-Explored Cruise Missile";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 240);

		strDisplay = "'G' for Guided Missile - Release Key for Explosion";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 280);
		...
		strDisplay = "Move Your Mouse and Click to Throw A Bomb";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 360);

		strDisplay = "Eat Yellow Floater to Power-Up Shields and Eat Blue Floater to Power-Up Ships";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 400);
